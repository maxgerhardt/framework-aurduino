 ...
 *     IfxCcu6_Timer_Config TimerChannelConfig;
 *     IfxCcu6_Timer_initModuleConfig(&TimerChannelConfig, &MODULE_CCU60);
 *
 ...
 *     TimerChannelConfig.base.frequency = 400000;
 *
 *     // configure the period of the timer
 *     TimerChannelConfig.base.period = 100;
 *
 *     // configure the waiting time in case of delayed T13 start in sync with T12
 *     TimerChannelConfig.base.waitingTime = 0;
 *
 *     // select the timer that needs to be started
 *     TimerChannelConfig.timer = IfxCcu6_TimerId_t12;
 *
 ...
 *     TimerChannelConfig.synchronousOperation = TRUE;
 *
 *     // configure the clock for internal mode
 *     TimerChannelConfig.clock.t12ExtClockEnabled   = FALSE;
 *     TimerChannelConfig.clock.t12ExtClockInput     = NULL_PTR;
 *     TimerChannelConfig.clock.t12countingInputMode = IfxCcu6_CountingInputMode_internal;
 *
 *     // configure the selcted timer block
 *     TimerChannelConfig.timer12.countMode     = IfxCcu6_T12CountMode_edgeAligned;
 *     TimerChannelConfig.timer12.counterValue     = 0;
 *
 *     // configure the interrupts
 *     TimerChannelConfig.interrupt.interruptSource = IfxCcu6_InterruptSource_t12PeriodMatch;
 *     TimerChannelConfig.interrupt.serviceRequest  = IfxCcu6_ServiceRequest_2;
 *     TimerChannelConfig.interrupt.priority        = IFX_INTRPRIO_CCU6;
 *     TimerChannelConfig.interrupt.typeOfService   = IfxSrc_Tos_cpu0;
 *
 *     // configure input and output triggers
 *     TimerChannelConfig.trigger.t12ExtInputTrigger   = IfxCcu60_T12HRB_P00_7_IN;
 *     TimerChannelConfig.trigger.t13ExtInputTrigger   = NULL_PTR;
 *     TimerChannelConfig.trigger.extInputTriggerMode  = IfxCcu6_ExternalTriggerMode_risingEdge;
 *     TimerChannelConfig.trigger.t13InSyncWithT12     = FALSE;
 *
 *     // initialize the module
 *     IfxCcu6_Timer_initModule(&timer, &TimerChannelConfig);
 * \endcode
 *
 *
