...
#elif defined(ARDUINO_ARCH_AURIX)
        static void init() {
                // Should be initialized by the user manually for now
            SPI_SS::SetDirWrite();
            SPI_SS::Set();
            USB_SPI.begin(BOARD_SPI_SS0);
            USB_SPI.beginTransaction(BOARD_SPI_SS0, SPISettings(12000000, MSBFIRST, SPI_MODE0)); // MJB 12MHz for now
        }
#elif !defined(SPDR)
        static void init() {
                SPI_SS::SetDirWrite();
...
#elif defined(ARDUINO_ARCH_AURIX)
typedef SPi< P62, P63, P61, P10 > spi;  // MJB
#else
...
void MAX3421e< SPI_SS, INTR >::regWr(uint8_t reg, uint8_t data) {
        ...
        c[0] = reg | 0x02;
        c[1] = data;
        //USB_SPI.transfer(BOARD_SPI_SS0, c, 2);  // Avoid using buffered transfer
        USB_SPI.transfer(BOARD_SPI_SS0, reg | 0x02);
        USB_SPI.transfer(BOARD_SPI_SS0, data);
...
uint8_t* MAX3421e< SPI_SS, INTR >::bytesWr(uint8_t reg, uint8_t nbytes, uint8_t* data_p) {
        ...
        data_p += nbytes;
#elif defined(ARDUINO_ARCH_AURIX)
        USB_SPI.transfer(BOARD_SPI_SS0, reg | 0x02);  // MJB Could use buffer transfer
       // USB_SPI.transfer(BOARD_SPI_SS0, data_p, nbytes);
       // data_p += nbytes;

        while(nbytes) {
                USB_SPI.transfer(BOARD_SPI_SS0, *data_p);
                nbytes--;
...
uint8_t MAX3421e< SPI_SS, INTR >::regRd(uint8_t reg) {
        ...
        SPI_SS::Set();
#elif defined(ARDUINO_ARCH_AURIX)
        USB_SPI.transfer(BOARD_SPI_SS0, reg);
        uint8_t rv = USB_SPI.transfer(BOARD_SPI_SS0, 0); // Send empty byte
        SPI_SS::Set();
...
uint8_t* MAX3421e< SPI_SS, INTR >::bytesRd(uint8_t reg, uint8_t nbytes, uint8_t* data_p) {
       ...
       // data_p += nbytes;
        while(nbytes) {
            *data_p++ = USB_SPI.transfer(BOARD_SPI_SS0, 0);
            nbytes--;
        }
#elif !defined(SPDR) // ESP8266, ESP32
        USB_SPI.transfer(reg);
