 ...
 */
extern void digitalWriteA( uint32_t dwPin, uint32_t dwVal ) ;  // Obsolete

/* 45ns pin access */
#define digitalWrite(ulPin, ulVal ) \
if(HIGH == (ulVal)) { IfxPort_setPinState(AURduinoPins[ulPin].port, AURduinoPins[ulPin].pinIndex, IfxPort_State_high); }\
else {IfxPort_setPinState(AURduinoPins[ulPin].port, AURduinoPins[ulPin].pinIndex, IfxPort_State_low);}
