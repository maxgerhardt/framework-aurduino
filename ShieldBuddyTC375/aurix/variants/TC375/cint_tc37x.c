...
static _FARDATA Hnd_arg * const Cdisptab[3] =
{
	Cdisptab_CPU0,
	Cdisptab_CPU1,
	Cdisptab_CPU2
...
int _install_int_handler(int intno, void (*inthandler)(int), int arg)
{
	...
	Cdisptab[CpuId][intno].hnd_handler = inthandler;
	Cdisptab[CpuId][intno].hnd_arg = arg;

	return 1;
}

int _install_remote_int_handler(int CoreNo, int intno, void (*inthandler)(int), int arg)
{
	if ((intno < 0) || (intno >= MAX_INTRS) || !_init_vectab_initialized[CoreNo])
		return 0;

	Cdisptab[CoreNo][intno].hnd_handler = inthandler;
	Cdisptab[CoreNo][intno].hnd_arg = arg;
...
extern int _FARDATA TriCore_trap_table[];
extern int _FARDATA TriCore_int_table[];
...
void _init_vectab(void)
{
	...
	/* set BTV register */
	unlock_wdtcon();
	/* activate short form of trap vector table */
	vaddr = (unsigned int)(&TriCore_trap_table);
	_mtcr(CPU_BTV, vaddr);
	lock_wdtcon();
	...
	{
		Cdisptab[CpuId][vecno].hnd_handler = __int_handler;
		Cdisptab[CpuId][vecno].hnd_arg = vecno;
	}
	...
	_init_vectab_initialized[CpuId] = 1u;
}
...
{
	__enable(); /* Re-enable interrupts asap */
...
#pragma GCC optimize "-O0"  /* Turn off optimisation to prevent loss of individual cases */
