...
class MCMCANClass
{
  ...
  private:
	...
	uint32 FindCanTxID_Offset(uint32 CAN_Id);
	uint32 FindCanRxID_Offset(uint32 CAN_Id);
	void ProcessFifo0Interrupt(void);

  public:
	MCMCANClass();  // Constructpr

	void CAN_Init(uint32 Baudrate);
	void CAN_RxInit(uint32 CAN_Id, uint32 AcceptanceMask, uint8 dlc, uint32 ExtFrame, uint8 MsgObj_Id);
	void CAN_TxInit(uint32 CAN_Id, uint32 AcceptanceMask, uint8 dlc, uint32 ExtFrame, uint8 MsgObj_Id);
	IfxCan_Status CAN_ReceiveMessage(uint32 CAN_Id, CANMessagePayloadType *msg1, uint8 dlc);
	int CAN_SendMessage(uint32 CAN_Id, CANMessagePayloadType *msg1, uint8 dlc);

};

extern MCMCANClass MCMCAN;
